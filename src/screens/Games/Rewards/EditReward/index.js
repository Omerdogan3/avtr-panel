...
function NewReward({open, setOpen, onSubmit, rewardId}) {
  ...
  const getReward = async ()=>{
     await api.getRewardById(rewardId).then((values)=>{
      if(values && values[0]){
        claimActions(allClaimTypes.indexOf(allClaimTypes.find(el=>el.claimName==values[0].claimType)))
        setClaimMultiple(values[0].claimMultiple)
        setComparison(values[0].comparison)
        setComparisonValue(values[0].comparisonValue)
        setAppId(values[0].gameId) 
        setNotifReward(values[0].notifReward)
        setRewardAmount(values[0].rewardAmount)
      }
    });
  }
  ...
  async function updateReward(){
    await api.updateReward(rewardId,{claimType, comparison, comparisonValue, claimMultiple, rewardAmount, notifReward})
    setOpen(false)
    onSubmit()
  }
  ...
  return (
    <Drawer 
      ...
      <Drawer.Header>
        <Drawer.Title>Edit Reward</Drawer.Title>
        <Drawer.Actions>
          <Button disabled={!appId} onClick={() => updateReward()} appearance="primary">
            Confirm
          </Button>
        </Drawer.Actions>
      </Drawer.Header>
